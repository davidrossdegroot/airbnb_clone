...
ActiveRecord::Base.transaction do
  ...
  # num_users = User.all.length + 1

  users = User.all

  users.each_with_index do |user, i|
    download_file = Net::HTTP.get_response(URI.parse("https://aerbnb-seeds.s3-us-west-1.amazonaws.com/photos/users/0#{i + 1}-user.jpg"))
    file = StringIO.new(download_file.body)
    user.photo.attach(io: file, filename: "#{i}-user.jpg")
    user.save!
  end
  ...
  (0...num_listings).each do |idx|
    (0...8).each do |photo_idx|
      ...
      if idx < 9
        # Full size Image
        download_file = Net::HTTP.get_response(URI.parse("https://aerbnb-seeds.s3-us-west-1.amazonaws.com/photos/listings/SF/0#{idx+1}-SF/original/0#{idx+1}-0#{photo_idx+1}-SF.jpg"))
        file = StringIO.new(download_file.body)
        listing.photos.attach(io: file, filename: "0#{idx+1}-0#{photo_idx+1}-SF.jpg")
        
        # Thumbnail
        download_file = Net::HTTP.get_response(URI.parse("https://aerbnb-seeds.s3-us-west-1.amazonaws.com/photos/listings/SF/0#{idx+1}-SF/thumbnail/0#{idx+1}-0#{photo_idx+1}-SF-thumb.jpg"))
        file = StringIO.new(download_file.body)
        ...
        # Thumbnail
        download_file = Net::HTTP.get_response(URI.parse("https://aerbnb-seeds.s3-us-west-1.amazonaws.com/photos/listings/SF/#{idx+1}-SF/thumbnail/#{idx+1}-0#{photo_idx+1}-SF-thumb.jpg"))
        file = StringIO.new(download_file.body)
